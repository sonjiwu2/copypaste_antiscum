...
describe('MissionsPage (Integration)', () => {
  ...
  it('renders the sidebar filters and catalog sections', async () => {
    ...
    expect(await screen.findByRole('heading', { name: /РЕКОМЕНДУЕМЫЕ МИССИИ/ })).toBeTruthy()
  ...
  it('allows filtering missions by role', async () => {
    ...
    await screen.findByRole('heading', { name: /РЕКОМЕНДУЕМЫЕ МИССИИ/ })
    const sidebar = container.querySelector('.mission-filters') as HTMLElement
    const buyerButton = within(sidebar).getByRole('button', { name: 'Покупатель' })
  ...
  it('opens the detail dialog when an unlocked card body is clicked and launches the mission', async () => {
    ...
    await screen.findByRole('heading', { name: /РЕКОМЕНДУЕМЫЕ МИССИИ/ })
    const openCard = screen.getByRole('button', { name: 'Ссылка на доставку' })
    await user.click(openCard)

    const dialog = screen.getByRole('dialog')
    expect(dialog).toBeTruthy()

    const beginBtn = within(dialog).getByRole('button', { name: 'Начать миссию' })
