...
# The frontend container serves the client and proxies the API, so the same
# origin must answer both. Set FRONTEND_URL='' to skip that part.
...
# The client is only useful if it is served together with a working API from
# one origin: that is what keeps the profile cookie same-site.
if [[ -n "$frontend_url" ]]; then
  page="$(curl --fail --silent --show-error "$frontend_url/")"
  if ! grep -q '<div id="root">' <<<"$page"; then
    fail 'frontend did not serve the application shell'
  fi

  proxied_readiness="$(curl --fail --silent --show-error "$frontend_url/readyz")"
  assert_json "$proxied_readiness" '.status == "ready"' 'frontend did not proxy /readyz to the backend'

  proxied_catalog="$(curl --fail --silent --show-error     --cookie "$owner_cookie" --cookie-jar "$owner_cookie"     "$frontend_url/api/v1/scenarios")"
  assert_json "$proxied_catalog" '(.scenarios | length) > 0' 'frontend did not proxy the scenario catalog'

  # Deep links must open the client, not an nginx 404: routes live in the hash,
  # but the browser still requests the path.
  deep_link_status="$(curl --silent --show-error --output /dev/null --write-out '%{http_code}'     "$frontend_url/missions")"
  if [[ "$deep_link_status" != '200' ]]; then
    fail "frontend deep link returned HTTP $deep_link_status instead of 200"
  fi

  echo "Frontend smoke passed: client and API answer on $frontend_url"
fi
