...
- отдельный frontend origin потребует credentialed CORS, которого сейчас нет.
...
- frontend и production hosting пока не входят в репозиторий;
- нет учётных записей и восстановления потерянной анонимной cookie;
- нет встроенного rate limiting;
- нет credentialed CORS для отдельного frontend origin;
